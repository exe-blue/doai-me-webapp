...
const Utils = {
    ...
    generateUniqueFilename: function(jobId, actionType, ext) {
        ...
        const safeJobId = this.sanitizeFilename(jobId || 'unknown');
        const safeActionType = this.sanitizeFilename(actionType || 'action');
        ...
        const seqStr = String(this._sequenceCounter).padStart(2, '0');
    ...
    generateUniqueFilePath: function(baseDir, jobId, actionType, ext) {
        const filename = this.generateUniqueFilename(jobId, actionType, ext);

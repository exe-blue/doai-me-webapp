#!/usr/bin/env node
// ============================================
// DoAi.Me YouTube Bot - Entry Point
// Command-line executable for YouTube worker
// ============================================

import * as dotenv from 'dotenv';
import { Logger, defaultLogger } from '@doai/worker-core';
import { YouTubeWorker, YouTubeWorkerConfig } from './YouTubeWorker';

// Load environment variables
dotenv.config();

// Parse configuration from environment
function parseConfig(): YouTubeWorkerConfig {
  const workerId = process.env.WORKER_ID || `youtube-${Date.now()}`;
...
// Create logger
const logger = defaultLogger.child('YouTubeBot');

// Main function
async function main(): Promise<void> {
  logger.info('Starting YouTube Bot Worker...');
  ...
  try {
    ...
    logger.info('YouTube Bot Worker is running', {
      workerId: config.workerId,

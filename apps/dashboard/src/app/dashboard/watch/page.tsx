...
import { useWatchQuery, watchKeys } from "@/hooks/queries";
...
export default function WatchPage() {
  ...
  const { data, isLoading: loading, error: queryError, refetch } = useWatchQuery({
    page,
    pageSize,
    status: statusFilter,
  ...
  const error = queryError ? '시청 목록을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.' : null;
    ...
    <div className="space-y-6">
      ...
      {error && !loading && (
        <div className="rounded-lg border-2 border-red-500 bg-red-50 dark:bg-red-950/20 p-6 text-center">
          ...
          <Button variant="outline" size="sm" onClick={() => refetch()} className="mt-3 border-2 border-foreground">
      ...
      {!error && (
        <WatchTable
          sessions={sessions}
          loading={loading}
          onRefresh={() => queryClient.invalidateQueries({ queryKey: watchKeys.all })}
        />
      )}

#!/usr/bin/env node
// ============================================
// DoAi.Me Health Bot - Entry Point
// Command-line executable for Health worker
// ============================================

import * as dotenv from 'dotenv';
import { Logger, defaultLogger } from '@doai/worker-core';
import { HealthWorker, HealthWorkerConfig } from './HealthWorker';

// Load environment variables
dotenv.config();

// Parse configuration from environment
function parseConfig(): HealthWorkerConfig {
  const workerId = process.env.WORKER_ID || `health-${Date.now()}`;
...
// Create logger
const logger = defaultLogger.child('HealthBot');

// Main function
async function main(): Promise<void> {
  logger.info('Starting Health Bot Worker...');
  ...
  const worker = new HealthWorker(config);
  ...
  try {
    ...
    logger.info('Health Bot Worker is running', {
      workerId: config.workerId,
...
export { HealthWorker, HealthWorkerConfig } from './HealthWorker';
export { HealthCheckHandler, ResetHandler } from './handlers';
export type {
  HealthCheckParams,
